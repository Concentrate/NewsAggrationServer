...
const RE_FLASH_RECOMMEND_DATA_GAP = 2 * 60 * 1000; //推荐数据刷新间隔 ms 2min
...
async function getNewsWithFalaor(labelList, selectSql, everylimitCount) {
    var tmpRes = []
    ...
    console.log("查找数据数量 %d", tmpRes.length)
    return tmpRes
}
...
async function getRecommendAlgrithmnData(userInfo) {
    ...
    var t1 = await getNewsWithFalaor(mostPopKeyWord, seleNewsSql, limitNum)
    allResult = allResult.concat(t1)
    seleNewsSql = "select * from toutiao_news where tag=\"%s\" order by behot_time desc limit %d"
    t1 = await getNewsWithFalaor(mostPopTags, seleNewsSql, limitNum)
    allResult = allResult.concat(t1)
    ...
    t1 = await (mostPopLabels, selectNews, limitNum)
    allResult = allResult.concat(t1)
